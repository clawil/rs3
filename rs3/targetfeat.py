# AUTOGENERATED! DO NOT EDIT! File to edit: ../02_targetfeat.ipynb.

# %% auto #0
__all__ = ['SECONDARY_STRUCTURE_GROUPS', 'add_target_columns', 'get_position_features', 'get_one_aa_frac', 'get_aa_aromaticity',
...
# %% ../02_targetfeat.ipynb #50f28f47
import pandas as pd
from Bio.SeqUtils.ProtParam import ProteinAnalysis
import warnings

# %% ../02_targetfeat.ipynb #dda2f4f0
...
# Amino acid groups as Biopython defined them before version 1.82. These are
# pinned here rather than delegating to ProteinAnalysis.secondary_structure_fraction()
# because 1.82 changed that method in two ways at once: it corrected a long-standing
# bug (the method returned (Sheet, Turn, Helix) while documenting (Helix, Turn, Sheet))
# and it changed the groups themselves (helix VIYFWL -> EMALK, turn NPGS -> NPGSD,
# sheet EMAL -> VIYFWLT).
#
# The Rule Set 3 target models were trained on the older definition, so reproducing it
# is what keeps published scores reproducible -- this is not a workaround for a
# Biopython bug, it is the feature the models expect. Delegating to Biopython's current
# implementation instead shifts target scores by a mean of 0.109 and a maximum of 0.445,
# against a score standard deviation of 0.152, on 361 of 400 test guides, with no error.
SECONDARY_STRUCTURE_GROUPS = {'Helix': 'VIYFWL', 'Turn': 'NPGS', 'Sheet': 'EMAL'}


def get_aa_secondary_structure(feature_dict, analyzed_seq):
    """Get the fraction of amino acids that tend to be in a helix, turn or sheet

    Uses the pre-1.82 Biopython amino acid groups the models were trained on, so
    the result does not depend on the installed Biopython version. See
    ...
    for feature, residues in SECONDARY_STRUCTURE_GROUPS.items():
        feature_dict[feature] = sum(aa_counts[residue] / length for residue in residues)

# %% ../02_targetfeat.ipynb #a05cf2a9
